...
export function getDbStories(): DbStory[] {
...
export function getDbStoryBySlug(slug: string): DbStory | undefined {
  const db = initializeDatabase();
  try {
...
export function addStory(story: Omit<DbStory, 'id' | 'createdAt'>): DbStory {
    const db = initializeDatabase();
    const now = new Date().toISOString();
    ...
    const newStory = getDbStoryById(info.lastInsertRowid as number);
    if (!newStory) {
...
function getDbStoryById(id: number): DbStory | undefined {
    const db = initializeDatabase();
...
export function addComment(comment: Omit<Comment, 'id' | 'createdAt'>): Comment {
    ...
    const newComment = getCommentById(info.lastInsertRowid as number);
    if (!newComment) {
