...
export interface DbStory {
    id: string; // The slug will be the ID/key in Firebase
    slug: string;
    title: string;
    author: string;
    summary: string;
...
export async function getComments(): Promise<Comment[]> {
    ...
    if (snapshot.exists()) {
        ...
        // Convert the Firebase object into an array and sort by date
        return Object.keys(commentsData)
            ...
            .map(c => ({...c, createdAt: new Date(c.createdAt)})); // Convert timestamp to Date object for component
    }
    return [];
}
...
export async function addComment(comment: Omit<Comment, 'id' | 'createdAt'>): Promise<Comment> {
    ...
    // To return the full comment, we need to fetch the timestamp value
    ...
    return {
        ...
        createdAt: savedComment.createdAt, // Keep as timestamp
    };
}
...
export async function addStory(story: Omit<DbStory, 'id' | 'createdAt'>): Promise<DbStory> {
    const db = getDatabase();
    // Use the slug as the key for the story
